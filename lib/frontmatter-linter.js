...
const rules = {
  Stage: {
    ...
    messages: {
      enum: `must be one of the RFC Stages: "${stages.join(
        '", "'
      )}" (See https://github.com/Mesitis/rfcs#stages)`,
    },
  },
  'Start Date': {
    type: 'date',
  ...
  'RFC PR': {
    ...
    allowEmpty: false,
    conform(url) {
      let regex = /https:\/\/github.com\/Mesitis\/rfcs\/pull\/\d+(\/|$)$/i;
      return regex.test(url);
    },
    messages: {
      conform:
        'must be the URL for the original pull request on Mesitis/rfcs, for example: https://github.com/Mesitis/rfcs/pull/123',
    },
  },
  'Release Date': {
    type: 'date',
